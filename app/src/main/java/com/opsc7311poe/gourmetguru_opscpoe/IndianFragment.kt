...
class IndianFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_indian, container, false)
        ...
        recyclerView = view.findViewById(R.id.rvIndianRecipes)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())



        // Initialize the adapter with an empty list and set click listener
        recipeAdapter = RecipeAdapter(listOf(), "Indian") { recipe ->
            navigateToAPICuisineRecipeDetailsFragment("Indian", recipe.name)
        }
    ...
    private fun fetchIndianRecipes() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val recipeNames = apiService.getRecipesByCuisine("Indian")
                ...
                for (recipeName in recipeNames) {
                    val recipeDetails = apiService.getRecipeDetails("Indian", recipeName)
                ...
                Log.e("IndianFragment", "Error fetching recipes", e)
            }
        }
    }
