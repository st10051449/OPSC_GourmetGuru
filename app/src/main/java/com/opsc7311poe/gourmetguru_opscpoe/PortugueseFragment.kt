...
class PortugueseFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_portuguese, container, false)
        ...
        recyclerView = view.findViewById(R.id.rvPortugueseRecipes)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())


        // Initialize the adapter with an empty list and set click listener
        recipeAdapter = RecipeAdapter(listOf(), "Portuguese") { recipe ->
            navigateToAPICuisineRecipeDetailsFragment("Portuguese", recipe.name)
        }
        recyclerView.adapter = recipeAdapter

        fetchPortugueseRecipes()

        return view
    }

    private fun fetchPortugueseRecipes() {
        val apiService = ApiClient.createService(RecipesApiService::class.java)

        CoroutineScope(Dispatchers.IO).launch {
            try {
                val recipeNames = apiService.getRecipesByCuisine("Portuguese")
                ...
                for (recipeName in recipeNames) {
                    val recipeDetails = apiService.getRecipeDetails("Portuguese", recipeName)
                ...
                Log.e("PortugueseFragment", "Error fetching recipes", e)
            }
        }
    }
