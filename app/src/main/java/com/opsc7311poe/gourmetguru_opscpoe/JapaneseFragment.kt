...
private lateinit var btnBack: ImageView
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_japanese, container, false)
        ...
        recyclerView = view.findViewById(R.id.rvJapaneseRecipes)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())


        // Initialize the adapter with an empty list and set click listener
        recipeAdapter = RecipeAdapter(listOf(), "Japanese") { recipe ->
            navigateToAPICuisineRecipeDetailsFragment("Japanese", recipe.name)
        }
    ...
    private fun fetchJapaneseRecipes() {
        val apiService = ApiClient.createService(RecipesApiService::class.java)

        CoroutineScope(Dispatchers.IO).launch {
            try {
                val recipeNames = apiService.getRecipesByCuisine("Japanese")
                ...
                for (recipeName in recipeNames) {
                    val recipeDetails = apiService.getRecipeDetails("Japanese", recipeName)
                ...
                Log.e("JapaneseFragment", "Error fetching recipes", e)
            }
        }
    }
